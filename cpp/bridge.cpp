...
// True when the current g_prefix_tokens range covers any multimodal (image)
// chunk. Image chunks embed as non-token positions in the KV cache, so the
// token ids in g_prefix_tokens across those positions are not meaningful for
// prefix comparison. Any subsequent lcw_start must do a full re-eval rather
// than trust the prefix vector across a media boundary.
bool g_prefix_has_media = false;
...
std::vector<llama_token> tokenize(const std::string & text) {
    // Speculative single pass: most chat turns fit well under 1024 tokens, so
    // issue one call with a generously sized buffer. Only if that proves too
    // small do we fall back to the standard measure-then-write path.
    ...
    // Buffer was too small (or empty input). written is the negative required
    // size; round-trip once with the exact capacity.
    ...
    const int32_t written2 = llama_tokenize(
        g_vocab,
        text.data(),
        static_cast<int32_t>(text.size()),
        tokens.data(),
    ...
    tokens.resize(static_cast<size_t>(written2));
    return tokens;
}
...
// Reports whether the loaded model exposes a built-in chat template. The
// text-only chat formatter path uses this to decide between the real
// template (lcw_format_chat_mm, preferred) and the hardcoded ChatML fallback
// (lcw_format_chat_multi). mmprojLoaded() is the wrong gate for that routing
// because a model can have a chat template without a vision projection (and
// vice versa).
EMSCRIPTEN_KEEPALIVE
int32_t lcw_model_has_chat_template() {
    if (g_model == nullptr) {
        return 0;
    }
    return llama_model_chat_template(g_model, nullptr) != nullptr ? 1 : 0;
}
...
int32_t lcw_eval_image(
    ...
    // chunks embed as non-token positions in the KV cache while g_prefix_tokens
    // holds llama_token ids, so a token-id comparison across a media boundary
    // is meaningless. Reset the KV cache and the prefix state so the next
    // lcw_start re-evaluates the prompt from scratch. The text generated by
    // the subsequent lcw_generate_chunk calls is recorded in g_prefix_tokens,
    // but g_prefix_has_media stays true so the next turn knows not to trust
    // the comparison across this image boundary.
    {
        llama_memory_t mem = llama_get_memory(g_context);
        if (mem != nullptr) {
            llama_memory_clear(mem, true);
    ...
    // Record the number of evaluated positions, but DO NOT fill g_prefix_tokens
    // across the image-chunk positions: image chunks map to non-token KV slots,
    // so there are no llama_token ids to store there. Storing a placeholder
    // (the prior resize(n_past) left stale data) would poison prefix
    // comparison. Leave g_prefix_tokens empty; the chunked generate loop
    // appends only real text tokens (post-image) as it decodes. The
    // g_prefix_has_media flag tells lcw_start to skip prefix reuse anyway.


    g_bench_prompt_tokens = static_cast<int32_t>(n_past);

    // Reserve room for the upcoming generation tokens so the per-token
    // push_back in lcw_generate_chunk does not trigger a vector realloc mid
    // decode. g_prefix_tokens is empty here (the image positions carry no
    // token ids), but the post-image text tokens will be appended.
    g_prefix_tokens.reserve(static_cast<size_t>(g_max_tokens));
...
int32_t lcw_format_chat_multi(
    ...
    while (true) {
        ...
        if (record_len > 0) {
            const char * sep_ptr = static_cast<const char *>(
                std::memchr(record, '\x1f', record_len));
            const char * role_ptr = record;
            size_t role_len;
            ...
            if (sep_ptr == nullptr) {
                ...
                role_len = static_cast<size_t>(sep_ptr - record);
                content_ptr = sep_ptr + 1;
                content_len = record_len - role_len - 1;
            }

            const bool is_system = role_len == 6 &&
                std::strncmp(role_ptr, "system", 6) == 0;
            const bool is_assistant = !is_system && role_len == 9 &&
                std::strncmp(role_ptr, "assistant", 9) == 0;
            ...
            prompt.append(content_ptr, content_len);
            prompt += "<|im_end|>\n";
            record_count += 1;
        }
    ...
    if (static_cast<int32_t>(prompt.size()) > output_capacity) {
        // Return negative required-size so the caller can size its buffer and
        // retry. Do NOT set_error here: this is a recoverable flow control,
        // not a hard failure.
...
int32_t lcw_format_chat_mm(
    ...
    if (needed > output_capacity) {
        // Return negative required-size so the caller can grow the buffer and
        // retry. Do NOT set_error: this is a recoverable flow control.
        return -needed;
    }
...
int32_t lcw_start(
    ...
    // When the prior turn was multimodal the token ids in g_prefix_tokens do
    // not span the image-chunk positions (they were never recorded; see
    // lcw_eval_image), so a token-id comparison is meaningless. Force a full
    // re-evaluation by treating the common prefix as zero and clearing the
    // KV cache + stale prefix state.
    size_t common = 0;
    if (g_prefix_has_media) {
        llama_memory_t mem = llama_get_memory(g_context);
        if (mem != nullptr) {
            llama_memory_clear(mem, true);
        }
        ...
        for (; common < n; ++common) {
            if (g_prefix_tokens[common] != prompt_tokens[common]) {
                break;
            }
        }
    }
    ...
    // g_prefix_tokens grows by new_token_count + g_max_tokens over the run.
    // Reserve once so the per-token push_back in lcw_generate_chunk never
    // triggers a vector realloc (and the associated full-buffer memcpy) mid
    // decode.
    g_prefix_tokens.reserve(g_prefix_tokens.size() + new_token_count + static_cast<size_t>(g_max_tokens));
